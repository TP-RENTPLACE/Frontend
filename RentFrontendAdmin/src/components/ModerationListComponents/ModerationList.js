...
const filteredAds = adsData.filter((ad) => {
  ...
  return (
    (ad.title && ad.title.toLowerCase().includes(q)) ||  // Проверка для title
    (ad.category && ad.category.toLowerCase().includes(q)) ||  // Проверка для category
    (ad.price && ad.price.toLowerCase().includes(q)) ||  // Проверка для price
    (ad.address && ad.address.toLowerCase().includes(q)) ||  // Проверка для address
    (ad.owner && ad.owner.toLowerCase().includes(q))  // Проверка для owner
  );
});
                ...
                <svg
                  ...
                  onClick={(e) => {
                    ...
                    if (clickX < 43 && currentPage > 1) {
                      setCurrentPage((prev) => prev - 1); // Влево
                    } else if (clickX >= 43 && currentPage < totalPages) {
                      setCurrentPage((prev) => prev + 1); // Вправо
                    }
                  }}
